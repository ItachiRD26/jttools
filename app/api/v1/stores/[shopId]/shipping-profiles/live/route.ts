// LOCATION: app/api/v1/stores/[shopId]/shipping-profiles/live/route.ts
// GET /api/v1/stores/{shopId}/shipping-profiles/live
...
export async function GET(
  ...
  const auth      = await validateRequest(apiKey, "stores/shipping-profiles/live");
  ...
  const res = await fetch(`${ETSY_BASE}/application/shops/${shopId}/shipping-profiles`, {
    headers: {
      "x-api-key":     API_KEY(),
  ...
  return NextResponse.json({
    ...
    shipping_profiles: Array.isArray(results) ? results : [results],
  });
}