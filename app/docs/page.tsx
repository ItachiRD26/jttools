...
const ENDPOINTS: Record<string, Endpoint[]> = {
  ...
  -H "x-api-key: jt_YOUR_KEY" -d '{"title":"Updated Title","quantity":20}'` },
    ...
    { method: "GET",    path: "/listings/batch",      description: "Fetch up to 100 listings by ID in a single request. Optional `includes` embeds related resources (Images, Inventory, Videos, Shipping, Translations, Shop, User, Manufacturers) — collapses 3N+ sync calls per listing into 1. Optional `shop_id` upgrades to OAuth so drafts return too (the public Etsy batch endpoint only returns active listings).",
      params: [{ name:"listing_ids",type:"string",required:true,description:"Comma-separated listing IDs (max 100)" },{ name:"includes",type:"string",required:false,description:"Comma-separated. Valid values: Shipping, Images, Shop, User, Translations, Inventory, Videos, Manufacturers" },{ name:"shop_id",type:"string",required:false,description:"If set, the call is authenticated with that shop's OAuth token — required to fetch draft listings (drafts come back empty without OAuth)." }],
