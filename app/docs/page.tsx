...
const ENDPOINTS: Record<string, Endpoint[]> = {
  ...
  -H "x-api-key: jt_YOUR_KEY" -d '{"title":"Updated Title","quantity":20}'` },
    ...
    { method: "PUT",    path: "/listings/inventory",  description: "Update SKU, price, and/or quantity on a listing's inventory items. The bridge fetches current inventory, merges any of {sku, price, quantity} changes by product_id, and PUTs the full shape back to Etsy — consumer only sends the fields they want to change. Every product row must include at least one of the three optional fields. Multi-offering products (per-region pricing): price + quantity apply to the FIRST offering only, preserving the others. Callers needing per-offering control can hit Etsy directly via the catch-all proxy. Idempotent — submissions where every field already matches Etsy return productsUpdated: 0 + skipped count without an Etsy write. If a submitted product_id isn't on the listing, the 404 response includes both `submitted` and `actual` arrays for a clean consumer-side diff. Pre-listings_w grants surface as STORE_SCOPE_OUTDATED with a reconnect hint instead of an opaque 502.", plan: "Pro+", note: "Requires Etsy shop connection (listings_w scope). Shops connected before listings_w was added must reconnect.",
      params: [
        ...
        { name:"products",   type:"array",  required:true,  description:"Body field. Array of { product_id: integer (required), sku?: string, price?: number, quantity?: integer }. Pull product_ids from GET /listings/inventory. Each row must include at least one of sku/price/quantity." }
      ],
      example: `curl -X PUT "https://jeterdev.tools/api/v1/listings/inventory?listing_id=1234567890&shop_id=61004439" -H "x-api-key: jt_YOUR_KEY" -H "Content-Type: application/json" -d '{"products":[{"product_id":987654321,"sku":"MUG-BLU-S","price":24.99},{"product_id":987654322,"quantity":50}]}'`,
    ...
    { method: "GET",    path: "/listings/variation-images", description: "Per-variation image mapping — which image_id is bound to which property_id/value_id combo on a listing (e.g. 'Color=Black' → image_id 5523110099001). Pair with /listings/images to resolve image_id → URL.", plan: "Pro+", note: "Requires Etsy shop connection.",
      params: [{ name:"listing_id",type:"string",required:true,description:"Etsy listing ID" },{ name:"shop_id",type:"string",required:true,description:"Etsy shop ID" }],
    ...
    { method: "GET",    path: "/listings/batch",      description: "Fetch up to 100 listings by ID in a single request. Optional `includes` embeds related resources — collapses N+ sync calls per listing into 1. Optional `shop_id` upgrades to OAuth so drafts return too (the public Etsy batch endpoint only returns active listings) AND is REQUIRED when includes contains `Inventory` or `Shipping` (as of July 29 2026 Etsy moved those to dedicated authenticated endpoints; the bridge fans out internally and merges the results so the response shape stays the same as before).",
      params: [{ name:"listing_ids",type:"string",required:true,description:"Comma-separated listing IDs (max 100)" },{ name:"includes",type:"string",required:false,description:"Comma-separated. Valid values: Images, Shop, User, Translations, Videos, Personalization, BuyerPrice, Inventory, Shipping. `Manufacturers` was removed on July 29 2026 and is silently dropped." },{ name:"shop_id",type:"string",required:false,description:"If set, the call is authenticated with that shop's OAuth token — required to fetch draft listings, or when includes contains Inventory / Shipping." }],
      example: `curl "https://jeterdev.tools/api/v1/listings/batch?listing_ids=1234567890,9876543210&includes=Images,Inventory&shop_id=61004439" -H "x-api-key: jt_YOUR_KEY"`,
      response: `{"count":2,"results":[{"listing_id":1234567890,"title":"Handmade Ceramic Mug","state":"draft","images":[{"listing_image_id":5523110099001,"rank":1,"url_570xN":"https://i.etsystatic.com/.../570xN.jpg",...}],"inventory":{"products":[...]},...},{"listing_id":9876543210,...}]}` },
  ],
